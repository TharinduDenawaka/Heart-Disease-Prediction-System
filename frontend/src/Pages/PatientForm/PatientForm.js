...
//     axios
//       .post("http://localhost:5000/api/data/sendData", formData)
//       .then((response) => {
...
const InputForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const numericFormData = Object.keys(formData).reduce((acc, key) => {
      if (formData[key] === "") {
        acc[key] = null;  // or you might want to set a default value
    ...
    axios
      .post("http://localhost:5000/api/data/sendData", dataToSend)
      .then((response) => {
        alert(`Prediction: ${response.data.prediction}`);
      })
      .catch((error) => {
